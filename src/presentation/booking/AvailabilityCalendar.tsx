...
export function AvailabilityCalendar({ selectedDate, onSelectDate }: Props) {
  ...
  return <div className="rounded-[2rem] border border-stone-200 bg-white p-5 shadow-[0_18px_60px_-30px_rgba(28,25,23,0.35)] md:p-7">
    ...
    <div className="relative mt-6 min-h-64"><div className={`grid grid-cols-7 gap-2 md:gap-3 ${loading ? "animate-pulse opacity-50" : ""}`} role="grid" aria-busy={loading} aria-label={`Disponibilidade de ${monthLabel}`}>
    ...
    </div>{loading && <div className="absolute inset-0 grid place-items-center"><div className="flex items-center gap-2 rounded-full bg-white/95 px-4 py-2 text-sm font-medium text-stone-600 shadow-sm"><span className="size-4 animate-spin rounded-full border-2 border-emerald-200 border-t-emerald-700" />Carregando disponibilidade...</div></div>}</div>
